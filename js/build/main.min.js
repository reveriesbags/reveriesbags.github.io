...
(function (require) {
	...
	var pathTojQuery = '//ajax.googleapis.com/ajax/libs/jquery/';

	// jq 2.x does not support IE > 9
	if('querySelector' in document &&
		 'localStorage' in window &&
		 'addEventListener' in window) {
		pathTojQuery += '2.0.3/jquery.min';
	} else {
		pathTojQuery += '1.10.2/jquery.min';
	}

	require.config({
		baseUrl: 'js',
		paths: {
			'jquery': [pathTojQuery, 'vendor/jquery.min'],
			'transparency': 'vendor/transparency.min.js',
...
require([
	...
	'lightbox'
		...
		String.prototype.camelize = function() {
			...
			return tmp.replace(/-+(.)?/g, function(match, chr) {
				return chr ? chr.toUpperCase() : '';
			});
		};

		function downloadTemplate (templateToLoad) {
			$.ajax({
				url: 'templates/' + templateToLoad + '.json?' + Math.random(),
				cache: true,
				dataType: 'json'
			}).done(function(data) {
				// $(document).ready(function() {
				/*jshint -W069 */
				$('#templateContainer').render(data['bags'], directives);
				$('img.lazy').lazyload({
					effect : 'fadeIn'
				});

				if (lsAvailable) {
					localStorage.setItem(templateToLoad + 'template',
						 JSON.stringify(data));
		...
		if ($body.hasClass('bags')) {
			var templateToLoad = $body.hasClass('children') ?
				'children' : 'adults',
				directives = {
					row: {
						'data-name': function() {
						...
						'data-colour': function () {
							return this.colour;
						}
					},
					lazy: {
						'data-original': function() {
							return 'img/bags/assets/' +
								templateToLoad.capitalize() +
								'/' + this.name.camelize() + '480.jpg';
						}
					},
					'lightbox-link': {
						href: function() {
							return 'img/bags/assets/' +
								templateToLoad.capitalize() +
								'/' + this.name.camelize() + '1024.jpg';
						},
						'data-lightbox': function() {
							return this.name.camelize();
						}
					}
				};

			var lsAvailable = 'localStorage' in window,
				lastUpdateTime = lsAvailable &&
					localStorage.getItem('lastUpdateTime');

			if (lsAvailable &&
				 +(new Date()) - lastUpdateTime <= 1209600 &&
				 localStorage.getItem(templateToLoad + 'template')) {
				var data = JSON.parse(localStorage.getItem(templateToLoad +
					 'template'));
				...
				$('img.lazy').lazyload({
				 ...
				 +(new Date()) - lastUpdateTime > 1209600 &&
				  localStorage.getItem(templateToLoad + 'template')) {
				localStorage.clear();
				downloadTemplate(templateToLoad);
			} else {
				downloadTemplate(templateToLoad);
			}
		}
